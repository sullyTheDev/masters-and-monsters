...
export class CreateAdventureComponent extends SmartListener implements OnInit {
  ...
  ngOnInit() {
    ...
    this.db.collection<CharacterAttribute>('character-attributes', col => col.where('base', '==', true).orderBy('order')).valueChanges().pipe(takeUntil(this.listener)).subscribe(x => {
      this.baseAttributes = x;
    });

    this.customizableAttributes.pipe(takeUntil(this.listener)).subscribe(x => {
      const formAttr = {};
      x.map(attr => {
